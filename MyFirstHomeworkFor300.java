...
public class MyFirstHomeworkFor300 {
	 ...
	 */
	public static double minValue (double[] list) {
		double lstMin = list[0];
		for (int i = 1; i < list.length; i++){
			if (list[i] < lstMin){
				lstMin = list[i];
			}
		}
		return lstMin; //TODO: fix this
	}
	...
	public static int distanceBetweenMinAndMax (double[] list) {
		...
		for (int i = 1; i < list.length; i++){
			...
			else if (list[i] > list[indexMax]){
				indexMax = i;
			}
		}
		return Math.abs(indexMax - indexMin); //TODO: fix this
		// return indexMax < indexMin ? indexMin - indexMax : indexMax - indexMin
	}
