...
public class MySecondHomework {
	...
	public static double sum (double[] a) {
		return sumHelper(a, 0, 0); // TODO
	}
	private static double sumHelper(double[] list, int i, double result) {
		if (i >= list.length) return 0;
		double add = sumHelper(list, i + 1, list[i]);
		return list[i] + add;
	}
	...
	public static void reverse (double[] a) {
		reverseHelper(a, 0, a.length-1); // TODO
	}
	
	private static void reverseHelper(double[] list, int low, int high){
		if (low < high) {
			double lowVal = list[low];
			double highVal = list[high];
			list[high] = lowVal;
			list[low] = highVal; 
			reverseHelper(list, low+1, high-1);
			}
	}
